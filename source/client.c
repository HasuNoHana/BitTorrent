...
void *listenSection(void *userAddr) {
    ...
    while (1) {
        ...
        if (acceptedSocket = acceptConnection(socketToListen, client) == 1) {
            break;
        } else {
            ...
            getDataFromDifferentUser(acceptedSocket, fileName, true);
            ...
            while (fgets(sendData, 1024, file) != NULL) {
                printf("%s\n", sendData);
                sendDataToDifferentUser(acceptedSocket, sendData, 1024, true);
                bzero(sendData, 1024);
            }
            fclose(file);
            printf("File %s sent!\n", fileName);
